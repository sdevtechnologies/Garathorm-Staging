    ...
    <div class="py-12">
        ...
        <div class="mx-auto sm:px-6 lg:px-8">
            <div class="d-flex justify-content-between">
                ...
                <a class="btn btn-light-green" href="{{ route('industry.create') }}"><i class="fa-solid fa-plus pe-1"></i>Add Industry References and Best Practices</a>
            </div>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-3">
                <div class=" mx-auto px-4 py-4">
                    ...
                    <div class="d-flex flex-row">
                        <a class="btn btn-primary m-1" id="AllButton" href="{{ route('industry.index')}}" >All</a> 
                        <a class="btn btn-primary m-1" id="whatsNewButton" href="{{ route('industry.whatsnewindex')}}" >What's New</a> 
                        <a class="btn btn-primary m-1" id="viewModeButton"><i class="fa-regular fa-eye pe-2"></i>View/Edit</a>
                        <form id="copySelectedForm" action="{{route('industry.copySelected') }}" method="POST">
                            @csrf
                        ...
                        <form id="deleteSelectedForm" action="{{ route('industry.deleteSelected') }}" method="POST">
                            @csrf
                            @method('DELETE')
                    ...
                    <table class="w-full mt-4 table">
                        ...
                        <tr>
                                ...
                                <th class="yh-category">@sortablelink('category.name','Category')</th>
                                <th class="th-related">Related Category</th>
                                <th class="th-col-description">Description
                                </th>
                                
                                <th class="th-publisher">@sortablelink('published','Publish')
                                </th>
                                <th class="th-publisher-name">@sortablelink('publisher.name','Publisher')</th>
                                <th class="th-date">@sortablelink('date_incident','Date of Incident')</th>
                            </tr>
                        </thead>
                        <tbody>
                            @include('industry.table_body')
                        </tbody>
                    </table>
                    
                    {{ $industries->appends(\Request::except('page'))->render() }}
